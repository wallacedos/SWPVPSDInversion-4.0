function [mInv,fValueSequence,fValueSequenceData, fValueSequenceModel, mAccepted] = rayleighDCPSDIter(ObsInfo,ModelInfo,InvParameterInfo)
%   Summary of this function goes here.
%   [mInv,fValueSequence,fValueSequenceData, fValueSequenceModel, mAccepted] = rayleighDCPSDIter(ObsInfo,ModelInfo,InvParameterInfo)
...
%                 layer, it's also a row vector.), 'mReference' is the predifined
%                 reference model, default as 0,full 0 vector is most smooth
...
%                the percent perturbation about the current model), 'maxIter'
%                (the maximum iterations of inversion),'misfitType' (the choice
%                of type for misfit function, 'L1' and 'L2' are the alternative
%                choice), 'epsilon' is a hyper-parameter for model
...
%
%   f = 5:100;
%   m = [150 450 600 6 10];den = [2000 2000 2000];pv = calcmulti(f,m(1:3),m(4:5),2.45*m(1:3),den);
%   pvObserved = pv(:,1);
%   Wd = ones(length(f),1);
%
%   ObsInfo.f = f;
...
%   ObsInfo.Wd = Wd;
%   ObsInfo.modeSN = [1];
%   ModelInfo.Bound = [50 500;50 1000;300 1000;0.5 30;0.5 30];
%   ModelInfo.Ini = [400 400 400 8 8];
%   ModelInfo.den = den;
%   ModelInfo.vpdvs = 2.45*ones(1,3);
%   ModelInfo.mReference = zeros(1,length(m));
...
%
%  You can find the principles of our method in any book about optimization
%  methods.
%
%  Please cite our references, if you use the program to publish an article.
%
%  History:
...
%          of the implicit function theorem, 8/28/2025
%          
%
%  Author(s): Yan Yingwei
%  Copyright: 2021-2030
%  Revision:  4.0  Date: 8/28/2025
%
% 
...
vpdvsInputState = isfield(ModelInfo,"vpdvs");
if vpdvsInputState
    vpdvs = ModelInfo.vpdvs; % the ratio vector of P-wave and S-wave velocity for each layer
else
    nn = floor((length(mCurrIni)+1+0.1)/2);
    vpdvs = 2.45*ones(1,nn);
...
VPIni = vs2vp(VSIni,vpdvs);
pvPredictedCal = calcmulti(freq,VSIni,HIni,VPIni,den,maxModeSN);
...
for k=1:maxIter
    ...
    pvPredictedMatrix(:,:,:) = 0;
    [gk,~,HessDiagElementK] = calcRayleighDCInvGradHess(ObsInfo,ModelInfo,mCurrIni,fCurrValue,epsilon,mReference,misfitType,stateId);
    ...
    parfor iTrial=1:trialNum
        ...
        VPTemp = vs2vp(VSTemp,vpdvs);
        pvPredicted = calcmulti(freq,VSTemp,HTemp,VPTemp,den,maxModeSN);
